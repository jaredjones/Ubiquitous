...
- (void)viewDidLoad {
    ...
    self.navigationController.title = @"Prior Tip's Made";
    self.title = @"Prior Tip's Made";
    ...
    if ([WCSession isSupported]) {
        ...
        NSLog(@"iPhone WCSession Activated");
    }else {
        NSLog(@"iPhone WCSession NOT SUPPORTED");
    }
...
- (void)session:(WCSession *)session didReceiveMessage:(NSDictionary<NSString *,id> *)message replyHandler:(void (^)(NSDictionary<NSString *,id> * _Nonnull))replyHandler{
    NSNumber *num = [message objectForKey:@"monetary"];
    
    dispatch_async(dispatch_get_main_queue(), ^{
        [_tipArray addObject:num];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSNumber *num = [_tipArray objectAtIndex:indexPath.row];
    
    
    NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
    [formatter setNumberStyle:NSNumberFormatterCurrencyStyle];
    NSString *currency = [formatter stringFromNumber:num];
